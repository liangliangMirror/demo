    ...
    <div class="_3titDnO9z79JXkaOV-6j8-">
      <div class="_2m6ykQWMTX3_dW7Ab-XsY8">
        <div
          ...
          :class="title.isok?'_350JA45HCnWxLcngmBwzjl':''"
          @click="ajaxs"
        >
          <router-link to="/" class="XojVIq9qzHwkS-H5hJLWh">广州市第八十九中学</router-link>
        </div>
        <div class="_1GdfNwKnUZUXB2ckSYH1m8" @click="dianji">
          <input placeholder="请输入商家或商品名称" v-model="title.title">
        </div>
      </div>
      <div>
        <xxxx :title="title"/>
...
import xxxx from "@/components/navs.vue";
@Component({
  components: {
    xxxx
  },
  ...
  methods: {
    dianji() {
      this.title.isok = !this.title.isok;
    },
    ajaxs() {
      this.$ajax
        .get(
          "https://www.easy-mock.com/mock/5d109ad4d2f7c32907262335/example/list"
        )
        .then(res => {
          console.log(res);
        })
